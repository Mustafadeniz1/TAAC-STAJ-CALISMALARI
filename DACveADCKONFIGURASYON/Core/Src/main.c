...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
    ...
    if (HAL_DAC_SetValue(&hdac1, DAC_CHANNEL_1, DAC_ALIGN_12B_R, dac_val) != HAL_OK) // 12 BİT DAC İÇİN ORTA OFFSET NOKTASI DEGERİ 2048
        Error_Handler();
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  if (HAL_ADC_PollForConversion(&hadc2, 10) != HAL_OK)      Error_Handler(); // timeout süresini bekler
	     adc_val = HAL_ADC_GetValue(&hadc2);   // debug’da izlemek için

	  if (dac_val > 4095) dac_val = 4095;              // 12-bitlik sınır değeri
